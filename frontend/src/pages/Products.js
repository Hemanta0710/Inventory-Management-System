...
import { getProducts, createProduct, updateProduct, deleteProduct,
  getProductBarcode, getSuppliers } from '../services/api';
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
              {['Name', 'SKU', 'Category', 'Supplier', 'Qty', 'Unit Price', 'Status', 'Actions'].map(h => (
            ...
            {filtered.map(p => {
              ...
              return (
                <tr key={p.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  ...
                  <td style={{ padding: '12px 16px' }}>${Number(p.unitPrice).toFixed(2)}</td>
                  <td style={{ padding: '12px 16px' }}>
                    <span style={{
